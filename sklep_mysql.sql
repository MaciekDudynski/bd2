...
CREATE FUNCTION getHashVal (p_in VARCHAR(50))
  RETURNS VARCHAR(50) RETURN MD5(p_in);
...
BEGIN
INSERT INTO ADRESY (MIASTO, ULICA, NR_BUDYNKU, NR_LOKALU, MIASTO_POCZTOWE, KOD_POCZTOWY)
  VALUES (miasto, ulica, nr_budynku, nr_lokalu, poczta, kod_pocztowy);
SET @adresy_id = LAST_INSERT_ID();
INSERT INTO KONTAKTY (NR_TEL, EMAIL)
  VALUES (telefon, email);
SET @kontakty_id = LAST_INSERT_ID();
-- INSERT INTO KLIENCI (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_KLIENTA, NAZWISKO_KLIENTA) VALUES ( (SELECT ID_ADRES FROM ADRESY WHERE ROWID = (SELECT MAX(ROWID) from ADRESY)), (SELECT ID_KONTAKT FROM KONTAKTY WHERE ROWID = (SELECT MAX(ROWID) from KONTAKTY)), login, haslo, imie, nazwisko);
INSERT INTO KLIENCI (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_KLIENTA, NAZWISKO_KLIENTA)
  VALUES (@adresy_id, @kontakty_id, login, getHashVal(haslo), imie, nazwisko);
-- SET @klienci_id = LAST_INSERT_ID();
-- UPDATE KLIENCI SET KLIENCI.LOGIN = getHashVal(KLIENCI.LOGIN) WHERE KLIENCI.ID_KLIENT = @klienci_id;
-- UPDATE KLIENCI SET KLIENCI.MD5_HASLO = getHashVal(KLIENCI.MD5_HASLO) WHERE KLIENCI.ID_KLIENT = @klienci_id;
END$$
...
INSERT INTO KONTAKTY (NR_TEL, EMAIL)
  VALUES (telefon, email);
SET @kontakty_id = LAST_INSERT_ID();
-- INSERT INTO PRACOWNICY (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_PRACOWNIKA, NAZWISKO_PRACOWNIKA, STANOWISKO, DATA_ZATRUDNIENIA) VALUES ( (SELECT ID_ADRES FROM ADRESY WHERE ROWID = (SELECT MAX(ROWID) from ADRESY)), (SELECT ID_KONTAKT FROM KONTAKTY WHERE ROWID = (SELECT MAX(ROWID) from KONTAKTY)), login, haslo, imie, nazwisko, stanowisko, data_zatrudnienia);
INSERT INTO PRACOWNICY (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_PRACOWNIKA, NAZWISKO_PRACOWNIKA, STANOWISKO, DATA_ZATRUDNIENIA)
  VALUES (@adresy_id, @kontakty_id, login, getHashVal(haslo), imie, nazwisko, stanowisko, data_zatrudnienia);
-- SET @pracownicy_id = LAST_INSERT_ID();
-- UPDATE PRACOWNICY SET PRACOWNICY.LOGIN = getHashVal(PRACOWNICY.LOGIN) WHERE PRACOWNICY.PRACOWNIK_ID = @pracownicy_id;
-- UPDATE PRACOWNICY SET PRACOWNICY.MD5_HASLO = getHashVal(PRACOWNICY.MD5_HASLO) WHERE PRACOWNICY.PRACOWNIK_ID = @pracownicy_id;
END$$
...
INSERT INTO DOSTAWCY (ID_ADRES, ID_KONTAKT, NAZWA_DOSTAWCY)
  VALUES (@adresy_id, @kontakty_id, nazwa);
END$$
...
IN datar DATE)
BEGIN
INSERT INTO SZCZEGOLY_ZAMOWIENIA (ID_KLIENT, ID_PRACOWNIK, DATA_ZLOZENIA_ZAMOWIENIA, DATA_REALIZACJI_ZAMOWIENIA)
  VALUES ( (SELECT ID_KLIENT FROM KLIENCI WHERE IMIE_KLIENTA = imiek AND NAZWISKO_KLIENTA = nazwiskok), (SELECT ID_PRACOWNIK FROM PRACOWNICY WHERE IMIE_PRACOWNIKA = imiep AND NAZWISKO_PRACOWNIKA = nazwiskop), dataz, datar);
SET @szczegoly_zamowienia_id = LAST_INSERT_ID();
...
INSERT INTO ZAMOWIENIA (ID_SZCZEGOLY_ZAMOWIENIA, ILOSC, ID_PRODUKT)
  VALUES (@szczegoly_zamowienia_id, ilosc, (SELECT ID_PRODUKT FROM PRODUKTY WHERE NAZWA_PRODUKTU = produkt));
UPDATE PRODUKTY SET PRODUKTY.ILOSC = PRODUKTY.ILOSC - ilosc;
END$$
...
DROP PROCEDURE IF EXISTS ZALOGUJ_PRACOWNIKA$$
CREATE PROCEDURE ZALOGUJ_PRACOWNIKA(
IN login VARCHAR(50),
IN haslo VARCHAR(50))
BEGIN
SELECT LOGIN FROM PRACOWNICY WHERE LOGIN = login AND MD5_HASLO = getHashVal(haslo);
END$$

DROP PROCEDURE IF EXISTS ZALOGUJ_KLIENTA$$
CREATE PROCEDURE ZALOGUJ_KLIENTA(
IN login VARCHAR(50),
IN haslo VARCHAR(50))
BEGIN
SELECT LOGIN FROM KLIENCI WHERE LOGIN = login AND MD5_HASLO = getHashVal(haslo);
END$$
...
CREATE OR REPLACE VIEW WIDOK_KLIENCI AS SELECT ID_KLIENT, LOGIN, MD5_HASLO, IMIE_KLIENTA, NAZWISKO_KLIENTA, MIASTO, ULICA, NR_BUDYNKU, NR_LOKALU, MIASTO_POCZTOWE, KOD_POCZTOWY, NR_TEL, EMAIL  FROM KLIENCI JOIN ADRESY USING(ID_ADRES) JOIN KONTAKTY USING(ID_KONTAKT);
CREATE OR REPLACE VIEW WIDOK_PRACOWNICY AS SELECT ID_PRACOWNIK, LOGIN, MD5_HASLO, IMIE_PRACOWNIKA, NAZWISKO_PRACOWNIKA, STANOWISKO, DATA_ZATRUDNIENIA, MIASTO, ULICA, NR_BUDYNKU, NR_LOKALU, MIASTO_POCZTOWE, KOD_POCZTOWY, NR_TEL, EMAIL FROM PRACOWNICY JOIN ADRESY USING(ID_ADRES) JOIN KONTAKTY USING(ID_KONTAKT);
...
INSERT INTO KLIENCI (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_KLIENTA, NAZWISKO_KLIENTA)
VALUES ( 1, 1, 'Grzesio123', getHashVal('grzes'), 'Grzegorz', 'Grzeg');
INSERT INTO KLIENCI (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_KLIENTA, NAZWISKO_KLIENTA)
VALUES ( 2, 2, 'Marian123',  getHashVal('marian'), 'Marian', 'Lewy');

INSERT INTO PRACOWNICY (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_PRACOWNIKA, NAZWISKO_PRACOWNIKA, STANOWISKO, DATA_ZATRUDNIENIA)
VALUES ( 3, 3, 'BorysSklep', getHashVal('borys'), 'Borys', 'Borysiewicz', 'Sprzedawca', '12/10/16');
INSERT INTO PRACOWNICY (ID_ADRES, ID_KONTAKT, LOGIN, MD5_HASLO, IMIE_PRACOWNIKA, NAZWISKO_PRACOWNIKA, STANOWISKO, DATA_ZATRUDNIENIA)
VALUES ( 4, 4, 'KierownikSklep', getHashVal('kierownik'), 'Marek', 'Marecki', 'Kierownik', '23/01/14');
